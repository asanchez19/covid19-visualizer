	...
	<div class="container">
		<div style="height: 200px;" class="text-center">
			...
			<select v-model="country" @change="onCountryChange">
				<option v-for="element in getCountries" :key="element">
					{{ element }}
				</option>
			</select>
...
export default {
	...
	computed: {
		...
		getDates() {
			...
			return this.slice(
				this.dataSet['Costa Rica'].map((data) => data.date)
			)
		},

		/**
		 * Get the max number of cases in the selected type.
		 *
		 * @return {Number}
		 */
		getTypeMax() {
	...
	methods: {
		...
		getData() {
			...
			this.values.push(
				this.slice(
					this.dataSet['Costa Rica'].map((data) => data[this.type])
				)
			)

			this.values.push(
				this.slice(
					this.dataSet[this.country].map((data) => data[this.type])
				)
			)
		},
		...
		/**
		 * Handle the on change type event.
		 *
		 * @return {void}
		 */
		onTypeChange() {
